...
class bal:
    def __init__(self, root):
        self.root = root
        self.root.title(" update Balance ")
        self.root.geometry("1050x380+235+250")
        # variables
        self.name = StringVar()
        self.acc_no = StringVar()
        ...
        label1 = Label(self.root, text="Update Balance", font=("Times", 30, "bold"), bg="blue", fg="white",
                       bd=2, )
        label1.place(x=0, y=0, width=1050, height=50)
        ...
        label_amnt = Label(main_frame, text="Enter amount ", font=("Times New Roman", 12, "bold"),
                           padx=2, pady=6)
        label_amnt.grid(row=1, column=0, sticky=W)
        entry_amnt = Entry(main_frame, textvariable=self.dep, width=30, font=("Times", 10, "bold"))
        entry_amnt.grid(row=1, column=1)
        bt3 = Button(main_frame, text="update balance", command=self.update_bal, font=("arial", 16), height=1,
                     fg="blue", cursor="hand1")
        bt3.place(x=100, y=80)

    def update_bal(self):
        if self.acc_no.get == "" or self.dep.get == "":
